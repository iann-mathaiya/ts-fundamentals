...
function loanCalculator(principal: number, interestRate: number, months: 12): number {
    let interest = interestRate / 1200;   // Calculates the monthly interest rate
    let payment;
    return payment = Math.round((principal * interest / (1 - (Math.pow(1 / (1 + interest), months))))* 100) / 100;
}

console.log(loanCalculator(10000, 2, 12));
